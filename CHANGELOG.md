...
- Linux support. A `String(localized:)` shim covers error strings, and the
  ISO-8859 string-encoding helper imports `CoreFoundation` explicitly (it is not
  re-exported by `Foundation` on Linux). Apple platforms are unaffected.
