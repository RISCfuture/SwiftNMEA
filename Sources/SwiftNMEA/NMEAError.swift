...
extension NMEAError: LocalizedError {
  public var errorDescription: String? {
    if let fieldNumber {
      return String(
        localized: "Couldn’t parse NMEA data: \(localizedType). (field #\(fieldNumber))"
      )
    }
  ...
  private var localizedType: String {
    switch type {
      ...
      case .wrongSentenceNumber:
        return String(localized: "Sentence number exceeded total sentence count.")
      ...
      case .missingFormat:
        return String(localized: "Expected a sentence format that wasn’t received.")
    }
  }
}
