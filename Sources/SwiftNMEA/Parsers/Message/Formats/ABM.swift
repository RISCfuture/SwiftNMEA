...
class ABMParser: MessageFormat {
    ...
    func parse(sentence: ParametricSentence) throws -> Message.Payload? {
        let totalSentences = try sentence.fields.int(at: 0)!,
            ...
            MMSI = try sentence.fields.int(at: 3, optional: true),
            channel = try sentence.fields.enumeration(at: 4, ofType: AIS.BroadcastChannel.self, optional: true),
            messageID = try sentence.fields.enumeration(at: 5, ofType: AIS.MessageID.self, optional: true),
            data = try sentence.fields.string(at: 6)!,
            fillBits = try sentence.fields.int(at: 7)!
        ...
        do {
            let element = BufferElement(lastSentence: sentenceNumber, totalSentences: totalSentences, encapsulatedData: data, fillBits: fillBits),
    ...
    private func makePayload(recipient: Recipient, element: BufferElement) throws -> Message.Payload? {
        ...
        return .AISBinaryMessage(sequentialIdentifier: recipient.sequentialID,
                                 MMSI: recipient.MMSI,
