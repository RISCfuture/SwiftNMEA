...
class BBMParser: MessageFormat {
  ...
  func parse(sentence: ParametricSentence) throws -> Message.Payload? {
    ...
    let messageID = try sentence.fields.enumeration(
      at: 4, ofType: AIS.MessageID.self, optional: true)
    ...
    let recipient = zipOptionals(channel, messageID).map { channel, messageID in
      Recipient(
        sentence: sentence, channel: channel, messageID: messageID, sequentialID: sequentialID)
    }
  ...
  private struct Recipient: BufferRecipient {
    ...
    init(
      sentence: ParametricSentence, channel: AIS.BroadcastChannel, messageID: AIS.MessageID,
      sequentialID: Int
    ) {
