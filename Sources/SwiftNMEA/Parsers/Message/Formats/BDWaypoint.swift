...
class BDWaypointParser: MessageFormat {
  ...
  func parse(sentence: ParametricSentence) throws -> Message.Payload? {
    ...
    let magneticBearing = try sentence.fields.bearing(at: 7, valueType: .float, referenceIndex: 8)!
    let distance = try sentence.fields.measurement(
      at: 9, valueType: .float, unitAt: 10, units: lengthUnits)!
    let waypointID = try sentence.fields.string(at: 11)!

    switch sentence.format {
    case Format.bearingDistanceToWaypointDR:
      return .bearingDistanceToWaypointDR(
    ...
    case Format.bearingDistanceToWaypointGC:
      ...
      return .bearingDistanceToWaypointGC(
        observationTime: time,
        position: position,
        bearingTrue: trueBearing,
        bearingMagnetic: magneticBearing,
    ...
    case Format.bearingDistanceToWaypointRL:
      let mode = try sentence.fields.enumeration(at: 12, ofType: Navigation.Mode.self)!
      return .bearingDistanceToWaypointRL(
        observationTime: time,
        position: position,
        bearingTrue: trueBearing,
        bearingMagnetic: magneticBearing,
