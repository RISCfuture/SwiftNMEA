...
class DSEParser: MessageFormat {
    ...
    func parse(sentence: ParametricSentence) throws -> Message.Payload? {
        let totalSentences = try sentence.fields.int(at: 0)!,
            ...
            type = try sentence.fields.enumeration(at: 2, ofType: DSE.MessageType.self, optional: true),
            MMSI = try sentence.fields.int(at: 3, optional: true).map { $0 / 10 },
            fields = sentence.fields[4...]
        ...
        let messages = try fields.chunks(ofCount: 2).enumerated().map { index, pair in
            ...
            guard let message = SwiftDSE.parse(format: command, data: pair.last!, isQuery: type == .query) else {
        ...
        do {
            let element = BufferElement(lastSentence: sentenceNumber, totalSentences: totalSentences, messages: messages),
