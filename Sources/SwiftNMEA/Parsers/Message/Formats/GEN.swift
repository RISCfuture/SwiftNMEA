...
class GENParser: MessageFormat {
    ...
    func parse(sentence: ParametricSentence) throws -> Message.Payload? {
        let index = try sentence.fields.hex(at: 0, width: 4)!,
            ...
            chunksOpt = sentence.fields[2...],
            chunks = chunksOpt.compactMap(\.self)
        ...
        do {
            let data = try binaryParser.decode(chunks),
                recipient = DataBuffer.Recipient(sentence: sentence, timestamp: time)
    ...
    func flush(talker: Talker?, format: Format?, includeIncomplete: Bool) throws -> [any Element] {
        ...
        return buffer.flush(talker: talker, format: format, includeIncomplete: true).map { recipient, element in
