...
class GFAParser: MessageFormat {
  ...
  func parse(sentence: ParametricSentence) throws -> Message.Payload? {
    ...
    let semimajorStddev = try sentence.fields.measurement(
      at: 3, valueType: .float, units: UnitLength.meters)!
    let semiminorStddev = try sentence.fields.measurement(
      at: 4, valueType: .float, units: UnitLength.meters)!
    let orientation = try sentence.fields.bearing(at: 5, valueType: .float, reference: .true)!
    let altitudeStddev = try sentence.fields.measurement(
      at: 6, valueType: .float, units: UnitLength.meters)!
    let selectedAccuracy = try sentence.fields.measurement(
      at: 7, valueType: .float, units: UnitLength.meters)!
    let integrityStatusStr = try sentence.fields.string(at: 8)!
    ...
    guard let RAIMChar = integrityStatusStr.char(at: 0),
      ...
      let GICStatus = GNSS.IntegrityStatus(rawValue: GICChar)
    else {
