...
class GNSParser: MessageFormat {
  ...
  func parse(sentence: ParametricSentence) throws -> Message.Payload? {
    ...
    let position = try sentence.fields.position(
      latitudeIndex: (1, 2), longitudeIndex: (3, 4), altitudeIndex: (8, nil), optional: true,
      altitudeType: .float)
    ...
    let separation = try sentence.fields.measurement(
      at: 9, valueType: .float, units: UnitLength.meters, optional: true)
    let dAge = try sentence.fields.measurement(
      at: 10, valueType: .float, units: UnitDuration.seconds, optional: true)
