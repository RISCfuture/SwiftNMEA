  ...
  func parse(sentence: ParametricSentence) throws -> Message.Payload? {
    ...
    let satelliteData = try (3..<(sentence.fields.endIndex - 1)).chunks(ofCount: 4).compactMap {
      ...
      let elevation = try sentence.fields.measurement(
        at: chunk.index(after: chunk.startIndex), valueType: .integer, units: UnitAngle.degrees)!
      let azimuth = try sentence.fields.bearing(
        at: chunk.index(chunk.startIndex, offsetBy: 2), valueType: .integer, reference: .true)!
      ...
      return GNSS.SatelliteInView(
        id: id, position: .init(elevation: elevation, azimuth: azimuth), SNR: SNR)
    }
    ...
    if !satellites.keys.contains(sentence.talker) {
      ...
      satellites[sentence.talker] = .init(
        totalSatellites: totalSatellites, totalMessages: totalMessages)
    }
