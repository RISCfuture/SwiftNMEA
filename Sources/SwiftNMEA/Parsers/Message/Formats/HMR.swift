...
class HMRParser: MessageFormat {
  ...
  func parse(sentence: ParametricSentence) throws -> Message.Payload? {
    ...
    let setDifference = try sentence.fields.measurement(
      at: 2, valueType: .float, units: UnitAngle.degrees)!
    let difference = try sentence.fields.measurement(
      at: 3, valueType: .float, units: UnitAngle.degrees)!
    ...
    let sensor1 = HeadingSensor(
      id: sensor1ID, heading: heading1, isValid: valid1, deviation: deviation1)
    let sensor2 = HeadingSensor(
      id: sensor2ID, heading: heading2, isValid: valid2, deviation: deviation2)
