...
class HTCParser: MessageFormat {
  ...
  func parse(sentence: ParametricSentence) throws -> Message.Payload? {
    ...
    let rudderAngle = try sentence.fields.measurement(
      at: 1, valueType: .float, units: UnitAngle.degrees, optional: true)
    ...
    let turnMode = try sentence.fields.enumeration(
      at: 4, ofType: Steering.TurnControl.self, optional: true)
    let rudderLimit = try sentence.fields.measurement(
      at: 5, valueType: .float, units: UnitAngle.degrees, optional: true)
    let offHeadingLimit = try sentence.fields.measurement(
      at: 6, valueType: .float, units: UnitAngle.degrees, optional: true)
    let radius = try sentence.fields.measurement(
      at: 7, valueType: .float, units: UnitLength.nauticalMiles, optional: true)
    let rate = try sentence.fields.measurement(
      at: 8, valueType: .float, units: UnitAngularVelocity.degreesPerMinute, optional: true)
    let headingToSteer = try sentence.fields.bearing(
      at: 9, valueType: .float, referenceIndex: 12, optional: true)
    let offTrackLimit = try sentence.fields.measurement(
      at: 10, valueType: .float, units: UnitLength.nauticalMiles, optional: true)
    let track = try sentence.fields.bearing(
      at: 11, valueType: .float, referenceIndex: 12, optional: true)
