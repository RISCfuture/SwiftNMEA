...
class MEBParser: MessageFormat {
  ...
  func parse(sentence: ParametricSentence) throws -> Message.Payload? {
    ...
    let channel = try sentence.fields.enumeration(
      at: 3, ofType: AIS.BroadcastChannel.self, optional: true)
    let MMSI = try sentence.fields.int(at: 4, optional: true)
    let messageID = try sentence.fields.enumeration(
      at: 5, ofType: AIS.MessageID.self, optional: true)
    let messageIndex = try sentence.fields.int(at: 6, optional: true)
    let behavior = try sentence.fields.enumeration(
      at: 7, ofType: AIS.BroadcastBehavior.self, optional: true)
    let destMMSI = try sentence.fields.int(at: 8, optional: true)
    let dataFlag = try sentence.fields.enumeration(
      at: 9, ofType: AIS.BinaryDataStructure.self, optional: true)
    let sentenceType = try sentence.fields.enumeration(
      at: 10, ofType: SentenceType.self, optional: true)
    let data = try sentence.fields.string(at: 11)!
    let fillBits = try sentence.fields.int(at: 12)!

    let recipient = zipOptionals(MMSI, messageID).map { MMSI, messageID in
      Recipient(
        sentence: sentence, MMSI: MMSI, channel: channel, messageID: messageID,
    ...
    throws -> Message.Payload?
  {
    ...
    guard let messageID = element.messageID else {
      throw NMEAError(type: .missingRequiredValue, line: sentence, fieldNumber: 5)
    }
    guard let messageIndex = element.messageIndex else {
      throw NMEAError(type: .missingRequiredValue, line: sentence, fieldNumber: 6)
    }
    guard let broadcastBehavior = element.broadcastBehavior else {
      throw NMEAError(type: .missingRequiredValue, line: sentence, fieldNumber: 7)
    }
    guard let binaryStructure = element.binaryStructure else {
      throw NMEAError(type: .missingRequiredValue, line: sentence, fieldNumber: 9)
    }
  ...
  private struct Recipient: BufferRecipient {
    ...
    init(
      sentence: ParametricSentence, MMSI: Int, channel: AIS.BroadcastChannel?,
      messageID: AIS.MessageID, sequentialID: Int
    ) {
