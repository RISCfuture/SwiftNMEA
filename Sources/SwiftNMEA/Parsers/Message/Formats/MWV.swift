...
class MWVParser: MessageFormat {
    ...
    func parse(sentence: ParametricSentence) throws -> Message.Payload? {
        let angle = try sentence.fields.measurement(at: 0, valueType: .float, units: UnitAngle.degrees)!,
            reference = try sentence.fields.enumeration(at: 1, ofType: RelativeWindReference.self)!,
            speed = try sentence.fields.measurement(at: 2, valueType: .float, unitAt: 3, units: speedUnits)!,
            status = try sentence.fields.bool(at: 4)!
