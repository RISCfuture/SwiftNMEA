...
class NRMParser: MessageFormat {
  ...
  func parse(sentence: ParametricSentence) throws -> Message.Payload? {
    ...
    let frequency = try sentence.fields.enumeration(at: 1, ofType: NAVTEX.Frequency.self)!
    let coverageMask = try sentence.fields.enumeration(
      at: 2, ofType: NAVTEX.Mask.self, optional: true)
    let typeMask = try sentence.fields.enumeration(at: 3, ofType: NAVTEX.Mask.self, optional: true)
    let status = try sentence.fields.enumeration(at: 4, ofType: SentenceType.self)!

    return .NAVTEXReceiverMask(
      function: function,
      frequency: frequency,
