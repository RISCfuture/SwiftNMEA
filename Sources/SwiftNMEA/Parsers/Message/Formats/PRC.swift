...
class PRCParser: MessageFormat {
  ...
  func parse(sentence: ParametricSentence) throws -> Message.Payload? {
    ...
    let pitchMode = try sentence.fields.character(at: 5)!
    let location = try sentence.fields.enumeration(
      at: 6,
      ofType: Propulsion.Location.self,
      optional: true
    )
    let number = try sentence.fields.int(at: 7)!
    ...
    let RPMDemand: Propulsion.RPMValue =
      switch RPMMode {
        ...
        case "R":
          .value(
            try sentence.fields.measurement(at: 2, valueType: .float, units: .revolutionsPerMinute)!
          )
        case "V": .invalid
    ...
    let pitchDemand: Propulsion.PitchValue =
      switch pitchMode {
        ...
        case "D":
          .value(try sentence.fields.measurement(at: 4, valueType: .float, units: .degrees)!)
        case "V": .invalid
        default: throw sentence.fields.fieldError(type: .unknownValue, index: 4)
      }

    return .propulsionRemoteControl(
      leverDemandPosition: leverPosition,
