...
class RTEParser: MessageFormat {
  ...
  func parse(sentence: ParametricSentence) throws -> Message.Payload? {
    ...
    let mode = try sentence.fields.enumeration(
      at: 2, ofType: Navigation.RouteType.self, optional: true)
    let identifier = try sentence.fields.string(at: 3, optional: true)
    let waypoints = sentence.fields[4...].compactMap(\.self)
    ...
    do {
      ...
      let element = BufferElement(
        lastSentence: sentenceNumber, totalSentences: totalSentences, identifiers: waypoints)
