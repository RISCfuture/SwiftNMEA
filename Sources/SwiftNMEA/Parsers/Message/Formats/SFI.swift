  ...
  func parse(sentence: ParametricSentence) throws -> Message.Payload? {
    ...
    let frequencies = try (0..<6).compactMap { index in
      ...
      let freq = try sentence.fields.enumeration(
        at: freqIndex, ofType: Comm.Frequency.self, optional: true)
      let mode = try sentence.fields.enumeration(
        at: modeIndex, ofType: Comm.OperationMode.self, optional: true)
    ...
    let element = BufferElement(
      lastSentence: sentenceNumber, totalSentences: totalSentences, frequencies: frequencies)
