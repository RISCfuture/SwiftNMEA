...
class SSDParser: MessageFormat {
  ...
  func parse(sentence: ParametricSentence) throws -> Message.Payload? {
    ...
    let pointAValue = try sentence.fields.measurement(
      at: 2, valueType: .integer, units: UnitLength.meters, optional: true)
    let pointBValue = try sentence.fields.measurement(
      at: 3, valueType: .integer, units: UnitLength.meters, optional: true)
    let pointCValue = try sentence.fields.measurement(
      at: 4, valueType: .integer, units: UnitLength.meters, optional: true)
    let pointDValue = try sentence.fields.measurement(
      at: 5, valueType: .integer, units: UnitLength.meters, optional: true)
