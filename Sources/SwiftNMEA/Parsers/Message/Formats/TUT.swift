...
class TUTParser: MessageFormat {
    ...
    func parse(sentence: ParametricSentence) throws -> Message.Payload? {
        let source = try sentence.fields.enumeration(at: 0, ofType: Talker.self)!,
            totalSentences = try sentence.fields.int(at: 1)!,
            sentenceNumber = try sentence.fields.int(at: 2)!,
            identifier = try sentence.fields.int(at: 3, optional: true),
            translationCode = try sentence.fields.string(at: 4)!,
            body = try sentence.fields.string(at: 5)!
    ...
    private func makePayload(recipient: Recipient, element: BufferElement) throws -> Message.Payload {
        ...
        return .multiLanguageText(source: recipient.source,
                                  text: element.text,
                                  data: data,
            ...
            switch translationCode {
                ...
                case "1"..."16":
                    guard let part = Int(translationCode),
                          let encoding = String.Encoding.iso8859(part: part) else { return nil }
