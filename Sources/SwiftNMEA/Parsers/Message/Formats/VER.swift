...
class VERParser: MessageFormat {
  ...
  func parse(sentence: ParametricSentence) throws -> Message.Payload? {
    ...
    let vendorID = try sentence.fields.string(at: 3, optional: true)
    let uniqueID = try sentence.fields.string(at: 4)!
    let serialNumber = try sentence.fields.string(at: 5, optional: true)
  ...
  private func makePayload(recipient: Recipient, element: BufferElement) throws -> Message.Payload {
    ...
    guard let softwareRevision = element.softwareRevision else {
      throw VERErrors.missingField(index: 7)
    }
    guard let hardwareRevision = element.hardwareRevision else {
      throw VERErrors.missingField(index: 8)
    }
