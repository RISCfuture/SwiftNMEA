...
class VTGParser: MessageFormat {
    ...
    func parse(sentence: ParametricSentence) throws -> Message.Payload? {
        let courseTrue = try sentence.fields.bearing(at: 0, valueType: .float, referenceIndex: 1)!,
            courseMag = try sentence.fields.bearing(at: 2, valueType: .float, referenceIndex: 3)!,
            speedKts = try sentence.fields.measurement(at: 4, valueType: .float, unitAt: 5, units: speedUnits)!,
            speedKph = try sentence.fields.measurement(at: 6, valueType: .float, unitAt: 7, units: speedUnits)!,
            mode = try sentence.fields.enumeration(at: 8, ofType: Navigation.Mode.self)!

        return .groundSpeedCourse(courseTrue: courseTrue,
