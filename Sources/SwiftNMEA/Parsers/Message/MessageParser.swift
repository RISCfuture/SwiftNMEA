actor MessageParser {
    ...
    func parse(sentence: ParametricSentence) throws -> Message? {
        ...
        guard let payload = try parsers.lazy.compactMap({ try $0.parse(sentence: sentence) }).first else { return nil }
    ...
    func flush(talker: Talker?, format: Format?, includeIncomplete: Bool) throws -> [any Element] {
        try formatParsers.flatMap { try $0.flush(talker: talker, format: format, includeIncomplete: includeIncomplete) }
    }
}
