...
final class SixBitCoder: Sendable {
  func encode(_ value: Data) -> (String, UInt8) {
    ...
    for byte in value {
      ...
      while bitCount >= 6 {
        ...
        let hexad = (bitBuffer >> shift) & 0b111111
        let  // … convert them to a character …
    ...
    if bitCount > 0 {
      ...
      let char = hexadToChar(UInt8(hexad))
      result.unicodeScalars.append(char)
    }
  ...
  func encode(_ value: Data, chunkSize: Int) -> ([String], UInt8) {
    ...
    let chunks = encoded.chunks(ofCount: chunkSize).map { String($0) }
