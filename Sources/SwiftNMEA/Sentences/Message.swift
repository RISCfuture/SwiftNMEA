...
public struct Message: Element, Sendable, Codable, Equatable {
  ...
  public enum Payload: Sendable, Codable, Equatable {
    ...
    /**
     ...
     - Note: This sentence is deprecated in edition 6.0 and has been replaced by
     the (GNS) sentence
     ``GNSSFix(_:time:mode:numSatellites:HDOP:geoidalSeparation:DGPSAge:DGPSReferenceStationID:status:)``.
    ...
    /**
     ...
     are linked by the
     ``SafetyNET/MessageIdentification/uniqueMessageNumber`` data field
     generated by the receiving EGC Terminal, which is included in both
     sentences. When the
    ...
    /**
     ...
     sentence with the reason code set to ``NAKReason/accessDenied`` and the
     associated sentence shall not be processed.

     - Parameter protectedSentence: The following sentence formatter that is
     protected (for example
     ``engineTelegraph(time:type:position:subPosition:location:number:)``).
