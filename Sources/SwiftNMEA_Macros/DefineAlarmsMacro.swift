  ...
  var alarmSyntax: [DeclSyntax] {
    get throws {
      ...
      return [
        DeclSyntax(
          """
                  public enum \(raw: alarmEnumName): Sendable, Codable, Equatable, \
  ...
  var alarmEnumRawCaseSyntax: DeclSyntax {
    if noSubsystems {
      DeclSyntax(
        """
                    case \(id):
                        guard subsystem == nil, let alarm = \(raw: alarmEnumName)(rawValue: type) \
        else {
                            return nil
                        }
      ...
      DeclSyntax(
        """
                    case \(id):
                        if let subsystem = \(raw: subsystemsEnumName)(subsystem: subsystem, type: \
        type) {
