import Algorithms
import Foundation
import Testing

@testable import SwiftNMEA

@Suite("BitCoder")
struct BitCoderTests {
  @Test("encodes a bitwise format")
  func encodesABitwiseFormat() throws {
    var writer = BitWriter(size: 90)
    writer.write(0, bits: 2)  // 00
    writer.write(123, bits: 10)  // 0001111011
    writer.write(1234, bits: 12)  // 010011010010
    writer.write(155, bits: 12)  // 000010011011
    writer.write(1357, bits: 12)  // 010101001101
    writer.write(4095, bits: 12)  // 111111111111
    writer.write(0b100, bits: 3)  // 100
    writer.write(0, bits: 1)  // 0
    writer.write(16300, bits: 14)  // 11111110101100
    writer.write(1, bits: 1)  // 1
    writer.write(0, bits: 1)  // 0
    writer.write(0, bits: 2)  // 00
    writer.write(128, bits: 8)  // 10000000

    // 0000 0111    07
    // 1011 0100    B4
    // 1101 0010    D2
    // 0000 1001    09
    // 1011 0101    B5
    // 0100 1101    4D
    // 1111 1111    FF
    // 1111 1000    F8
    // 1111 1110    FE
    // 1011 0010    B2
    // 0010 0000    20
    // 00(00 00)    00

    let data = writer.data
    #expect(data[0] == 0x07)
    #expect(data[1] == 0xB4)
    #expect(data[2] == 0xD2)
    #expect(data[3] == 0x09)
    #expect(data[4] == 0xB5)
    #expect(data[5] == 0x4D)
    #expect(data[6] == 0xFF)
    #expect(data[7] == 0xF8)
    #expect(data[8] == 0xFE)
    #expect(data[9] == 0xB2)
    #expect(data[10] == 0x20)
    #expect(data[11] == 0x00)
  }

  @Test("decodes a bitwise format")
  func decodesABitwiseFormat() throws {
    let data = Data([0x07, 0xB4, 0xD2, 0x09, 0xB5, 0x4D, 0xFF, 0xF8, 0xFE, 0xB2, 0x20, 0x00])
    var reader = BitReader(data: data)

    // these are unsigned fields, read into unsigned types as the real callers do
    #expect((reader.read(bits: 2) as UInt) == 0)
    #expect((reader.read(bits: 10) as UInt) == 123)
    #expect((reader.read(bits: 12) as UInt) == 1234)
    #expect((reader.read(bits: 12) as UInt) == 155)
    #expect((reader.read(bits: 12) as UInt) == 1357)
    #expect((reader.read(bits: 12) as UInt) == 4095)
    #expect((reader.read(bits: 3) as UInt) == 0b100)
    #expect((reader.read(bits: 1) as UInt) == 0)
    #expect((reader.read(bits: 14) as UInt) == 16300)
    #expect((reader.read(bits: 1) as UInt) == 1)
    #expect((reader.read(bits: 1) as UInt) == 0)
    #expect((reader.read(bits: 2) as UInt) == 0)
    #expect((reader.read(bits: 8) as UInt) == 128)
  }

  @Test("sign-extends two's-complement reads into signed types")
  func signExtendsTwosComplementReadsIntoSignedTypes() throws {
    var signedReader = BitReader(data: Data([0x8F]))
    #expect((signedReader.read(bits: 4) as Int) == -8)  // 1000
    #expect((signedReader.read(bits: 4) as Int) == -1)  // 1111

    // the same bits read into an unsigned type are not sign-extended
    var unsignedReader = BitReader(data: Data([0x8F]))
    #expect((unsignedReader.read(bits: 4) as UInt8) == 8)
    #expect((unsignedReader.read(bits: 4) as UInt8) == 15)
  }
}
