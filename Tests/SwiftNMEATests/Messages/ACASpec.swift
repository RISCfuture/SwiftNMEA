        ...
        describe("8.3.5 ACA") {
            it("parses a sentence") {
                let parser = SwiftNMEA(),
                    inUseChanged = Date(timeIntervalSinceNow: -1000),
                    sentence = createSentence(
                        delimiter: .parametric, talker: .commVHF, format: .AISChannelAssignment,
                        fields: [0, 3712.12, "N", 12112.35, "W", 3615.09, "N", 12011.11, "W", 2, 5, 0, 12, 1, 2, 0, "C", 1, hmsFractionFormatter.string(from: inUseChanged)]),
                ...
                guard case let .AISChannelAssignment(sequenceNumber,
                                                     northeastCorner,
                                                     southwestCorner,
                                                     transitionZoneSize,
                                                     channelA,
                                                     ...
                                                     source,
                                                     inUse,
                                                     inUseChangedActual) = payload else {
