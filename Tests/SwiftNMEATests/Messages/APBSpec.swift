            ...
            it("parses a sentence") {
                let parser = SwiftNMEA(),
                    sentence = createSentence(
                        delimiter: .parametric, talker: .commVHF, format: .autopilotSentenceB,
                        fields: [
                            "A", "V",
                            12.3, "L", "N",
                            "V", "A",
                ...
                guard case let .autopilotSentenceB(LORANC_blinkSNRFlag, LORANC_cycleLockWarningFlag, crossTrackError, arrivalCircleEntered, perpendicularPassed, bearingOriginToDest, destinationID, bearingPresentPosToDest, headingToDest, mode) = payload else {
