            ...
            it("parses a sentence") {
                let parser = SwiftNMEA(),
                    ...
                    sentence = createSentence(
                        delimiter: .parametric, talker: .GPS, format: .GNSSFaultDetection,
                        fields: [hmsFractionFormatter.string(from: time), 1.2, 3.4, 5.6,
                                 35, 0.5, 1.5, 0.75,
                                 1, 5]),
                ...
                guard case let .GNSSFaultDetection(actualTime, latitudeError, longitudeError, altitudeError, failedSatellite, missProbability, biasEstimate, biasEstimateStddev) = payload else {
