...
struct `8.3.40 GEN` {
  ...
  @Test
  func `parses the example in the spec`() throws {
  ...
  @Test
  func `represents an interior null field as a gap`() throws {
    let parser = SwiftNMEA()
  ...
  @Test
  func `reports an error for a malformed (present but non-HEX) field`() throws {
    let parser = SwiftNMEA()
