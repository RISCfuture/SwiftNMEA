        ...
        describe("8.3.34 GFA") {
            it("parses a sentence") {
                let parser = SwiftNMEA(),
                    time = Date(timeIntervalSinceNow: -23),
                    sentence = createSentence(
                        delimiter: .parametric, talker: .GNSS, format: .GNSSAccuracyIntegrity,
                ...
                guard case let .GNSSAccuracyIntegrity(actualTime, HPL, VPL, semimajorStddev, semiminorStddev, semimajorErrorOrientation, altitudeStddev, selectedAccuracy, integrity) = payload else {
