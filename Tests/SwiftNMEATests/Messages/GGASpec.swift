  ...
  override static func spec() {
    describe("8.3.35 GGA") {
      it("parses a sentence") {
        ...
        guard
          case .GPSFix(
            let position, let actualTime, let quality, let numSatellites, let HDOP,
            let geoidalSeparation, let DGPSAge, let DGPSReferenceStationID) = payload
        else {
      ...
      it("parses a sentence from a STA8089FG") {
        ...
        guard
          case .GPSFix(
            let position, _, let quality, let numSatellites, let HDOP, let geoidalSeparation,
            let DGPSAge, let DGPSReferenceStationID) = payload
        else {
          fail("expected .GNSSAccuracyIntegrity, got \(payload)")
          return
        }
