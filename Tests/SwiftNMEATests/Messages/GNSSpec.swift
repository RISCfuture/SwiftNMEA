    ...
    override static func spec() {
        describe("8.3.37 GNS") {
            it("parses the first example from the spec") {
                ...
                sentence = applyChecksum(to: "$GNGNS,122310.2,3722.425671,N,12258.856215,W,DA,14,0.9,1005.543,6.5,5.2,23,S"),
                ...
                guard case let .GNSSFix(position, time, mode, numSatellites, HDOP, geoidalSeparation, DGPSAge, DGPSReferenceStationID, status) = message.payload else {
            ...
            it("parses the second example from the spec") {
                let parser = SwiftNMEA(),
                sentence = applyChecksum(to: "$GPGNS,122310.2,,,,,,7,,,,5.2,23,S"),
                data = sentence.data(using: .ascii)!,
                messages = try await parser.parse(data: data)
