            ...
            it("parses a sentence") {
                let parser = SwiftNMEA(),
                    ...
                    sentence = createSentence(
                        delimiter: .parametric, talker: .GPS, format: .GNSSPseudorangeNoise,
                ...
                guard case let .GNSSPseudorangeNoise(actualTime, rangeStddevRMS, errorSemimajorStddev, errorSemiminorStddev, errorOrientation, errorLatitudeStddev, errorLongitudeStddev, errorAltitudeStddev) = payload else {
                ...
                expect(errorOrientation).to(equal(.init(angle: .init(value: 4.4, unit: .degrees), reference: .true)))
