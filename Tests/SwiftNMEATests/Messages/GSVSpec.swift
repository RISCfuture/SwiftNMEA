        ...
        describe("8.3.41 GSV") {
            it("parses a sentence") {
                ...
                let parser = SwiftNMEA(),
                    sentences = [
                        createSentence(
                            delimiter: .parametric, talker: .GPS, format: .GNSSSatellitesInView,
                            fields: [2, 1, 7,
                                     "01", 11, 21, 31,
                                     "02", 12, 22, 32,
                                     "03", 13, 23, 33,
                        ...
                        createSentence(
                            ...
                            fields: [2, 2, 7,
                                     ...
                                     1]),
                        createSentence(
                            delimiter: .parametric, talker: .GLONASS, format: .GNSSSatellitesInView,
                            fields: [1, 1, 4,
                                     "65", 15, 25, 35,
                                     "66", 16, 26, 36,
                                     "67", 17, 27, 37,
                ...
                for i in 1...total {
                    ...
                    expect(satellites[i - 1].position.elevation).to(equal(.init(value: Double(10 + i), unit: .degrees)))
                    expect(satellites[i - 1].position.azimuth).to(equal(.init(degrees: Double(20 + i), reference: .true)))
                ...
                for i in 1...total {
                    ...
                    expect(satellites[i - 1].position.elevation).to(equal(.init(value: Double(i + 14), unit: .degrees)))
                    expect(satellites[i - 1].position.azimuth).to(equal(.init(degrees: Double(i + 24), reference: .true)))
