            ...
            it("parses a sentence") {
                let parser = SwiftNMEA(),
                    sentence = createSentence(
                        delimiter: .parametric, talker: .autopilotGeneral, format: .headingControlCommand,
                        fields: ["A", 5.5, "L",
                                 "R", "T",
                                 6.5, 2.0, 0.5, 1.5,
                                 180.1, 0.25, 190.5, "M",
                ...
                guard case let .headingControlCommand(heading, track, rudderAngle, override, mode, turnMode, rudderLimit, headingLimit, trackLimit, radius, rate, status) = payload else {
