    ...
    describe("8.3.55 MEB") {
      describe(".parse") {
        ...
        it("throws an error for a missing field") {
          ...
          let sentences = [
            createSentence(
              delimiter: .encapsulated, talker: .commVHF, format: .broadcastCommandMessage,
              fields: [
                2, 1, 1,
                1, nil, 1, 1, 1,
                9_876_543_210, 1, "R",
        ...
        it("flushes incomplete sentences") {
          ...
          guard
            case .broadcastMessage(
              let sequence, let AISChannel, let MMSI,
              let
                messageID, let messageIndex,
              let
                broadcastBehavior,
              let
                destinationMMSI,
              let
                binaryStructure,
              let
                sentenceType, let actualData) = message.payload
          else {
