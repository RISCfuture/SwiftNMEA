        ...
        describe("8.3.55 MEB") {
            describe(".parse") {
                it("parses a sentence") {
                    ...
                    data2 = "Each message must be no more than 82 characters. Each message must be no more than 82 characters.".data(using: .ascii)!,
                    (chunks1, fillBits1) = sixBit.encode(data1, chunkSize: 82),
                    (chunks2, fillBits2) = sixBit.encode(data2, chunkSize: 82)

                    let sentences1 = encapsulatedSentences(format: .broadcastCommandMessage,
                                                           from: chunks1,
                                                           fillBits: fillBits1,
                                                           sequenceID: 0,
                                                           otherFields: [0, 1234567890, 6, 3, 0, 9876543210, 1, "C"]),
                        sentences2 = encapsulatedSentences(format: .broadcastCommandMessage,
                                                           from: chunks2,
                                                           fillBits: fillBits2,
                                                           sequenceID: 1,
                                                           otherFields: [1, 9876543210, 14, 2, 1, nil, 0, "R"]),
                ...
                it("throws an error for a missing field") {
                    ...
                    let data = "Each message must be no more than 82 characters. Each message must be no more than 82 characters.".data(using: .ascii)!,
                    (chunks, fillBits) = sixBit.encode(data, chunkSize: 82),
                    sentences = [
                        createSentence(delimiter: .encapsulated, talker: .commVHF, format: .broadcastCommandMessage,
                                       fields: [2, 1, 1,
                                                1, nil, 1, 1, 1,
                                                9876543210, 1, "R",
                                                chunks[0], fillBits]),
            ...
            describe(".flush") {
                it("flushes incomplete sentences") {
                    ...
                    let data = "123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890".data(using: .ascii)!,
                    (chunks, fillBits) = sixBit.encode(data, chunkSize: 82)

                    let sentences = encapsulatedSentences(format: .broadcastCommandMessage,
                                                          from: chunks,
                                                          fillBits: fillBits,
                                                          sequenceID: 0,
                                                          otherFields: [0, 1234567890, 6, 3, 0, 9876543210, 1, "C"]),
                    ...
                    guard case let .broadcastMessage(sequence, AISChannel, MMSI,
                                                     messageID, messageIndex,
                                                     broadcastBehavior,
                                                     destinationMMSI,
                                                     binaryStructure,
                                                     sentenceType, actualData) = message.payload else {
                    ...
                    expect(destinationMMSI).to(equal(9876543210))
                    expect(binaryStructure).to(equal(.application))
                    expect(sentenceType).to(equal(.command))
                    expect(actualData).to(equal("1234567890123456789012345678901234567890123456789012345678901".data(using: .ascii)!))
                }
            }
            ...
            it("throws an error for a missing field") {
                ...
                let data = "Each message must be no more than 82 characters. Each message must be no more than 82 characters.".data(using: .ascii)!,
                (chunks, fillBits) = sixBit.encode(data, chunkSize: 82),
                sentence = createSentence(delimiter: .encapsulated, talker: .commVHF, format: .broadcastCommandMessage,
                                          fields: [2, 1, 1,
                                                   1, 1234567890, 1, 1, nil,
                                                   9876543210, 1, "R",
                                                   chunks[0], fillBits]),
                sentenceData = sentence.data(using: .ascii)!,
                parsed = try await parser.parse(data: sentenceData)
