    ...
    override static func spec() {
        describe("8.3.62 NRM") {
            it("parses the first example sentence") {
                ...
                data = Data("$INNRM,2,1,00001E1F,00000023,R*29\r\n".utf8),
                ...
                guard case let .NAVTEXReceiverMask(function, frequency, coverageAreaMask, messageTypeMask, status) = payload else {
            ...
            it("parses the second example sentence") {
                let parser = SwiftNMEA(),
                    data = Data("$INNRM,0,2,00001E1F,0FFFFFFF,R*5F\r\n".utf8),
                    messages = try await parser.parse(data: data)
