...
final class OSDSpec: AsyncSpec {
  override static func spec() {
    describe("8.3.75 OSD") {
      ...
      it("parses a sentence with all data fields null") {
        let parser = SwiftNMEA()
        let sentence = createSentence(
          delimiter: .parametric,
          talker: .integratedNavigation,
        ...
        expect(heading).to(beNil())
        expect(headingValid).to(beFalse())
        expect(course).to(beNil())
        expect(courseReference).to(beNil())
        expect(speed).to(beNil())
        expect(speedReference).to(beNil())
        expect(set).to(beNil())
        expect(drift).to(beNil())
      }
    }
  }
}
