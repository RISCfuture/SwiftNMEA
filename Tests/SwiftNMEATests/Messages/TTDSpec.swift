        ...
        describe("8.3.84 TTD") {
            describe(".parse") {
                it("parses a sentence") {
                    ...
                    let target = Data([0x07, 0xB4, 0xD2, 0x09, 0xB5, 0x4D, 0xFF, 0xF8, 0xFE, 0xB2, 0x20, 0x00]),
                        (chunks, fillBits) = sixBit.encode(target, chunkSize: 60)

                    let sentences = encapsulatedSentences(format: .trackedTargets,
                                                          from: chunks,
                                                          fillBits: fillBits,
                                                          sequenceID: 0,
                                                          otherFields: [],
                    ...
                    guard case let .trackedTargets(targets) = payload else {
                ...
                it("throws an error for an incorrect sentence number") {
                    ...
                    let sentences = [
                        createSentence(delimiter: .encapsulated, talker: .commVHF, format: .trackedTargets,
            ...
            describe(".flush") {
                it("flushes incomplete sentences") {
                    ...
                    let targets = Data([
                        ...
                        (chunks, fillBits) = sixBit.encode(targets, chunkSize: 60)
                    ...
                    let sentences = encapsulatedSentences(format: .trackedTargets,
                                                          ...
                                                          otherFields: [],
                                                          hex: true),
                        data = sentences[0...1].joined().data(using: .ascii)!
                    ...
                    guard case let .trackedTargets(targets) = message.payload else {
