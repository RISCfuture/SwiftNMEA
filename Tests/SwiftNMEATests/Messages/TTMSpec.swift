    ...
    describe("8.3.85 TTM") {
      it("parses a sentence") {
        ...
        let sentence = createSentence(
          delimiter: .parametric, talker: .radar, format: .trackedTarget,
          fields: [
            12,
        ...
        guard
          case .trackedTarget(
            let number, let distance, let bearing, let speed, let course, let CPADistance,
            let CPATime, let name, let status, let isReference, let actualTime, let acquisition) =
            payload
        else {
