  ...
  override static func spec() {
    describe("8.3.89 VBW") {
      it("parses the example from the spec") {
        ...
        guard
          case .speedData(
            let water, let waterValid, let ground, let groundValid, let sternTransverseWater,
            let sternTransverseWaterValid, let sternTransverseGround, let sternTransverseGroundValid
          ) = payload
        else {
