...
final class VDMSpec: AsyncSpec {
    override static func spec() {
        ...
        000100
        """,
                VDLBytes = VDLBinaryData
                    .replacing(.newlineSequence, with: "")
                    .chunks(ofCount: 8)
                    .map { UInt8($0, radix: 2)! },
                VDLData = Data(VDLBytes)
                ...
                it("parses a sentence") {
                    ...
                    let sentences1 = encapsulatedSentences(format: .VDLMessage,
                                                           from: chunks1,
                                                           fillBits: fillBits1,
                                                           sequenceID: 0,
                                                           otherFields: ["A"]),
                        sentences2 = encapsulatedSentences(format: .VDLMessage,
                                                           from: chunks2,
                                                           fillBits: fillBits2,
                                                           sequenceID: 1,
                                                           otherFields: ["B"]),
                ...
                it("parses a 62-character (46-byte) sentence when some fields are nil") {
                    ...
                    let data = "1234567890123456789012345678901234567890123456".data(using: .ascii)!,
                        ...
                        sentence = createSentence(delimiter: .encapsulated, talker: .commVHF, format: .VDLMessage,
                ...
                it("throws an error for an incorrect sentence number") {
                    ...
                    let data = "This data is exactly 62 characters long. This data is exactly ".data(using: .ascii)!,
                        ...
                        sentences = [
                            createSentence(delimiter: .encapsulated, talker: .commVHF, format: .VDLMessage,
                ...
                it("parses the first example from the spec") {
                    let parser = SwiftNMEA(),
                    sentence = applyChecksum(to: "!AIVDM,1,1,,A,1P000Oh1IT1svTP2r:43grwb05q4,0"),
                    sentenceData = sentence.data(using: .ascii)!,
                    messages = try await parser.parse(data: sentenceData)
            ...
            describe(".flush") {
                it("flushes incomplete sentences") {
                    ...
                    let data = "12345678901234567890123456789012345678901234567890123456789012".data(using: .ascii)!,
                        (chunks, fillBits) = sixBit.encode(data, chunkSize: 60)

                    let sentences = encapsulatedSentences(format: .VDLMessage,
                                                          from: chunks,
                                                          fillBits: fillBits,
                                                          sequenceID: 0,
                                                          otherFields: ["A"]),
