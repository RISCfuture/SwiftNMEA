        ...
        describe("8.3.106 ZDA") {
            it("parses the first example from the spec (corrected)") {
                let parser = SwiftNMEA(),
                    sentence = applyChecksum(to: "$GPZDA,234500.00,09,06,1995,-12,45"),
                ...
                let CHAT = TimeZone(secondsFromGMT: 12 * 60 * 60 + 45 * 60)!,
                    expectedDateComponents = DateComponents(timeZone: CHAT, year: 1995, month: 6, day: 10, hour: 12, minute: 30),
                    expectedDate = Calendar.current.date(from: expectedDateComponents)!
            ...
            it("parses the second example from the spec (corrected)") {
                let parser = SwiftNMEA(),
                    sentence = applyChecksum(to: "$GPZDA,013000.00,11,06,1995,10,30"),
                    data = sentence.data(using: .ascii)!,
                    messages = try await parser.parse(data: data)
                ...
                let KCT = TimeZone(secondsFromGMT: -10 * 60 * 60 - 30 * 60)!,
                    expectedDateComponents = DateComponents(timeZone: KCT, year: 1995, month: 6, day: 10, hour: 15, minute: 0),
                    expectedDate = Calendar.current.date(from: expectedDateComponents)!
