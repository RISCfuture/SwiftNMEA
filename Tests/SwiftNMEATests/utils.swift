...
func applyChecksum(to sentence: String) -> String {
    let delimiter = sentence.first!,
        rest = sentence.slice(from: 1),
        checksum = calculateChecksum(for: rest),
...
func createSentence(delimiter: Delimiter, talker: Talker, format: Format, fields: [Any?]) -> String {
    ...
    let sentence = ParametricSentence(delimiter: delimiter,
                                      talker: talker,
                                      format: format,
                                      fields: strFields)
...
func encapsulatedSentences(format: Format, from chunks: [String], fillBits: UInt8, sequenceID: Int, otherFields: [Any?], hex: Bool = false) -> [String] {
    ...
    return chunks.enumerated().map { index, chunk in
        ...
        return if index == 0 {
            createSentence(
                delimiter: .encapsulated, talker: .commVHF, format: format,
                fields: [totalSentences, lastSentence, sequenceID] + otherFields + [chunk, fillBits])
        } else {
            createSentence(
                delimiter: .encapsulated, talker: .commVHF, format: format,
                fields: [totalSentences, lastSentence, sequenceID] + Array(repeating: nil, count: otherFields.count) + [chunk, fillBits])
        }
    }
}
